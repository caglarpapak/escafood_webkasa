import { CheckMoveAction, CheckStatus, ContactType, Prisma, TransactionCategory, TransactionType, } from "@prisma/client";
...
const createAttachment = async (tx, file, userId) => {
...
export class CheckService {
    static async registerIn(payload) {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const attachment = await createAttachment(tx, attachmentFile, payload.createdById);
    ...
    static async issueCompanyCheck(payload) {
        ...
        await prisma.$transaction(async (tx) => {
            const attachment = await createAttachment(tx, payload.attachment, payload.createdById);
            const check = await tx.check.create({
                data: {
                    serialNo: payload.serialNo,
                    bank: payload.bank,
